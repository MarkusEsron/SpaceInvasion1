...
public class GameScreen implements Screen {
    ...
    public GameScreen (final MyGdxGame gam) {
        ...
        music = Gdx.audio.newMusic(Gdx.files.internal("SpaceBattle.mp3"));
        ...
        FreeTypeFontGenerator generator = new FreeTypeFontGenerator(Gdx.files.internal("BigSpace.ttf"));
        FreeTypeFontGenerator.FreeTypeFontParameter parameter = new FreeTypeFontGenerator.FreeTypeFontParameter();
        parameter.color = new Color(1, 1, 0, 1); // rgba от 0 до 1
        parameter.size = 40;
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.LEFT)) {
            spaceShip.dx -= 25 * Gdx.graphics.getDeltaTime();
        }
        ...
        game.batch.begin();
        for (int i = 0; i < 2; i++)
            game.batch.draw(imgStars, stars[i].x, stars[i].y, stars[i].width, stars[i].height);

        for (int i = 0; i < fragment.size; i++)
            game.batch.draw(imgFragment[fragment.get(i).type],
        ...
        for (int i = 0; i < enemy.size; i++)
            game.batch.draw(imgEnemy[enemy.get(i).type], enemy.get(i).x - enemy.get(i).width / 2,
                    enemy.get(i).y - enemy.get(i).height / 2,
                    enemy.get(i).width, enemy.get(i).height);

        for (int i = 0; i < shot.size; i++)
            game.batch.draw(imgShot, shot.get(i).x - shot.get(i).width / 2,
                    shot.get(i).y - shot.get(i).height / 2,
                    shot.get(i).width, shot.get(i).height);

        if (spaceShip.isAlive)
            game.batch.draw(imgSpaceShip, spaceShip.x - spaceShip.width / 2, spaceShip.y - spaceShip.height / 2,
                    spaceShip.width, spaceShip.height);

        GlyphLayout draw = game.font.draw(batch, "SCORE: " + score, 10, SCR_HEIGHT - 10);
        if (isSoundOn) game.batch.draw(imgSoundOn, SCR_WIDTH - 60, SCR_HEIGHT - 60, 50, 50);
        else game.batch.draw(imgSoundOff, SCR_WIDTH - 60, SCR_HEIGHT - 60, 50, 50);

        if (gameState == WAIT_GAME_RESTART) {
            game.font.draw(batch, "GAME OVER", 0, SCR_HEIGHT / 4 * 3, SCR_WIDTH, Align.center, false);
            for (int i = 0; i < nPlayers - 1; i++)
                game.font.draw(batch, (i + 1) + ". " + player[i].name + " - " + player[i].score, 0, SCR_HEIGHT / 4 * 3 - 50 - 40 * i, SCR_WIDTH, Align.center, false);
        }

        if (gameState == GAME_PAUSE)
            game.font.draw(batch, "PAUSE", 0, SCR_HEIGHT / 2, SCR_WIDTH, Align.center, false);
        game.batch.end();
    }
